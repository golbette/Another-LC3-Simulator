//simulator.c 
//Developed on c9.io, compiled and tested on UW-T CSSgate server
#include <stdio.h>
...
//	BSTR_Display(sub,1);
	...
	{
		BitString instr;
		BSTR_SetBits(&instr, program[i]);
		COMP_LoadWord(&comp, i, instr);
	}
	...
	/* following not instruction is   ADD Dest: R4, Source: R5, imm:5*/
    //BitString notInstr;
	//BSTR_SetBits(&notInstr,"0000100000000101");
	//COMP_LoadWord(&comp,0,notInstr);
	
	//Unsure of COMP_DISPLAY calls should remain in final code, keeping them here to be safe 
    COMP_Display(comp);
